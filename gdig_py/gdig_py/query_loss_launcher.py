...
def main():
    ...
    # Store intermediate files in factor directory
    filenames=[os.path.join(args.cache, str(datetime.datetime.timestamp(datetime.datetime.now()))+'.json') for i in range(args.nsubsets)]
    ...
    for idx, (fn, subset) in enumerate(zip(filenames, indices)):
        ...
        child = subprocess.Popen(['python3', os.path.join(file_dir, "query_loss_mapper.py"), '-v', f'cuda:{idx}', \
                                    ...
                                    '--layer', f'{args.layer}', '-lc', f'{args.layer_collection}'])
