...
def main():
    ...
    parser.add_argument('-c', '--cache')  # on/off flag
    ...
    parser.add_argument('-lc', '--layer_collection', type=str, default='bloom',
                      choices=['bloom', 'baichuan', 'llama'],
                      help='Type of layer collection to use')
    ...
    filenames=[os.path.join(args.cache, str(datetime.datetime.timestamp(datetime.datetime.now()))+'.kfac') for i in range(args.nsubsets)]
    ...
    for idx, (fn, subset) in enumerate(zip(filenames, indices)):
        ...
        child = subprocess.Popen(['python3', os.path.join(file_dir, "kfac_mapper.py"), '-v', f'cuda:{idx}', \
                                    '-idstart', f'{subset[0]}', '-idend', f'{subset[1]}', \
                                    '-o', f'{fn}', '-m', f'{args.model}', \
                                    '-t', f'{args.trials}', '-d', f'{args.data_path}', \
                                    '-k', f'{args.tokenizer}', '-lc', f'{args.layer_collection}'])
    ...
    for fn in filenames:
        with open(fn, 'rb') as f:
            pF = pickle.load(f)
            for key in pF.data.keys():
                pF.data[key] = (pF.data[key][0].to(device), pF.data[key][1].to(device))
            kfac_list.append(pF)
    reduce=kfac_list[0]
